...
var tugOfWarModel = function(){
  ...
  var populate_whoIsWhat(i, whois, whoiswhat) {
    var len = whois.length;
    if (i <= len && len != 0) {
      whoiswhat[i] = whois[i-1]["is"];
      populate_whoIsWhat(i+1, whois, whoiswhat);
    } else {
      return whoiswhat
    }
  }

    whoiswhat = populate_whoIsWhat(1, whoIs, whoIsWhat);
